...
| **Parser** | `scripts/sa_article_parser.py` | 4단계 fallback (내부 API → Jina → Playwright → curl_cffi) + 공식 티커 후보. dashboard 사본과 항상 byte-identical. |
...
│     2) parse_sa_article(url)   — 내부API → Jina → PW → curl_cffi    │
...
- ticker = 기사 이벤트에 **실질적으로 관련된** 상장 기업만("이 기사가 그 기업 주가에 의미 있는 정보인가?" 기준 — 당사자·계약상대·인수대상 등 포함, 단순 비교·나열·배경 언급 제외), 쉼표+공백 구분 (`NVDA, AMD`). 가장 주요한 기업 첫 번째. 비상장 기관(연준 등) 제외. 없으면 빈 문자열
...
## 파서 (sa_article_parser.py, v6)

**Fallback 순서 (내부 API 우선)**:
0. **SA 내부 API** (`/api/v3/news/{id}?include=primaryTickers`) — `parse_with_sa_api`. **1순위.**
   - `content` 필드 = 네비게이션·'Recommended For You' 노이즈 0의 순수 본문. Jina의 ~10000자 노이즈 덤프 대체.
   - `primaryTickers` = SA가 직접 태깅한 **공식 후보 종목**(심볼↔회사명 정확) → stderr `SA_TICKERS:` 라인으로 요약기에 전달.
   - **주의: 페이월 우회 아님.** 본문은 비로그인 프리뷰 ~300자로 다른 경로와 동일하게 잘림(모든 무인증 경로가 동일 바이트에서 절단 = 계정등급 서버사이드 게이팅). og 리드가 잘린 끝을 일부 보완.
   - `primaryTickers`는 ETF/테마 기사에서 과잉 태깅(예: 금 기사 → 금ETF 13개). 그래서 **그대로 채택하지 않고** 요약 프롬프트에 후보 화이트리스트로 주입 → LLM이 본문 기준 실질 관련만 선택. secondaryTickers는 너무 넓어(인덱스·인버스ETF·동종업체) 미사용.
   - `/news/{id}` 형식 아니거나(예: `/article/`) 본문 <80자면 None → 다음 폴백.
