...
- 요약 모델 설정(2026-09-26 사용자 지시, wm 설정 팝업 이식): 헤더 톱니 → 공용 `static/model-selector.{js,css}`(wm·yt와 글자 단위로 같게 유지) 계열 모드. 저장은 `sa_news.db` `app_settings.summary_config`, 규칙은 `summary_config.py`(`MODEL_CHOICES` — 지금은 **Grok 4.7 하나**). 파이프라인은 `sa_summarize_claude.summary_chain()`이 설정 순번대로 호출하고 순번 밖 모델로 폴백하지 않는다(예전 Claude 폴백 없음, 실패는 기존 재시도). grok 호출에 `-m`·`--reasoning-effort`를 넘긴다. 옛 `settings.SUMMARY_PRIMARY`는 삭제.
