...
def init_db():
    with get_conn() as conn:
        ...
        # 마이그레이션: article_body 컬럼 (파싱한 영어 원문 본문). 기존행은 NULL 유지.
        ...
        if ticker is not None:
            cur = conn.execute(
                ...
                UPDATE articles SET
                  ...
                  summary_details = ?, ticker_color = ?, parse_method = ?, article_body = ?,
                ...
                (
                    ...
                    ticker_color, parse_method, article_body, last_modified, article_id,
                ),
            )
        else:
            cur = conn.execute(
